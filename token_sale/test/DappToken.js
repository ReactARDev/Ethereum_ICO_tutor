...
contract('DappToken', function(accounts) {
	...
	it('initialize contract with the correct values', function() {
	...
	it('approve tokens for delegated trnasfer', function() {
		return DappToken.deployed().then(function(instance) {
			tokenInstance = instance;
			...
			assert.equal(receipt.logs[0].args._owner, accounts[0], 'logs the account the tokens are apporved from');
	...
	it('handles delegated token transfer', function(){
		return DappToken.deployed().then(function(instance){
			...
			assert(error.message.indexOf('revert') >= 0, 'cannot trnasfer value larger than balance');
			// try transfering something larger than the approved amount
			return tokenInstance.transferFrom(fromAccount, toAccount, 20, { from: spendingAccount })
		}).then(assert.fail).catch(function(error) {
			assert(error.message.indexOf('revert') >= 0, 'cannot trnasfer value larger than approved amount');
